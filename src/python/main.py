...
def generate_facts_from_name(df, output_file, fact_name, columns):
    ...
    try:
        ...
        print(f"Fatti `{fact_name}\\{len(columns)}` unici scritti in {output_file}")

    except csv_loader.ColumnNotFoundError as e:
        print(e)
    except Exception as e:
        print(f"Errore durante la scrittura dei fatti `{fact_name}\\{len(columns)}`: {e}")
...
def generate_facts():
    ...
    generate_facts_from_name(df, os.path.join(output_dir, 'corsi_di_studio.asp'), 'corso_di_studio', ['Cod. Corso di Studio'])
